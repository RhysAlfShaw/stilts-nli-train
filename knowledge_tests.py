...
# load trained model
model_path = (
    "/scratch/Rhys/stilts_models/gemma-2b-finetuned/final_model"  # Llama 3 model path
)
...
def parse_stilts_command(command_str: str) -> tuple[str, dict]:
    """Parses a STILTS command string into a tool and a dictionary of arguments."""
    # A simple pipe is a special case we need to handle by splitting
    if " | " in command_str:
        # For chained commands, we just normalize whitespace and compare directly for simplicity
        # A more advanced parser would be needed for a full component-wise comparison
        normalized_str = re.sub(r"\s+", " ", command_str).strip()
        return "chained_command", {"full_command": normalized_str}
    ...
    # The tool is everything before the first key=value pair
    first_arg_index = len(parts)
    for i, part in enumerate(parts):
        if "=" in part:
            first_arg_index = i
            break
    tool_parts = parts[:first_arg_index]
    ...
    for arg in parts[first_arg_index:]:
        if "=" in arg:
