...
class ModelBase:
    ...
    def register(cls, *names: str) -> Callable[[AnyModel], AnyModel]:
        ...
        def func(modelcls: AnyModel) -> AnyModel:
            model_type = (
                ModelType.VISION
                if modelcls.model_arch == gguf.MODEL_ARCH.CLIP_VISION
                else ModelType.TEXT
            )
            for name in names:
...
class VisionModel(ModelBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.model_arch != gguf.MODEL_ARCH.CLIP_VISION:
            raise TypeError(
                "VisionModel must be subclassed with model_arch = gguf.MODEL_ARCH.CLIP_VISION"
            )
        ...
        self.tensor_map = gguf.get_tensor_name_map(
            gguf.MODEL_ARCH.CLIP_VISION, self.block_count
        )
...
class Qwen2VLVisionModel(VisionModel):
    ...
    def set_gguf_parameters(self):
        ...
        if self.global_config["model_type"] == "qwen2_vl":
            ...
            self.gguf_writer.add_vision_projector_type(
